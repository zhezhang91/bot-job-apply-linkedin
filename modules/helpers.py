...
def get_chrome_major_version() -> int | None:
    '''
    Returns the installed Google Chrome major version (e.g. 149), or None if unknown.
    '''
    if sys.platform.startswith('win'):
        try:
            import winreg
        ...
        for app_dir in chrome_app_dirs:
            ...
            for entry in os.listdir(app_dir):
                if entry[0].isdigit() and os.path.isdir(os.path.join(app_dir, entry)):
...
def manual_login_retry(is_logged_in: callable, limit: int = 2) -> None:
    '''
    Function to wait for manual login without blocking popups
    '''
    print_lg("Waiting for LinkedIn login in the browser...")
    attempts = 0
    max_attempts = max(limit * 60, 120)
    while not is_logged_in():
        sleep(3)
        attempts += 1
        if attempts >= max_attempts:
            print_lg("Timed out waiting for manual login.")
            return
    print_lg("Login detected!")
